...
def configurar_paginas_de_erro(app: FastAPI):
    ...
    async def erro_http_exception(
        request: Request, ex: HTTPException, usuario: Usuario = Depends(obter_usuario_logado)
    ):
        return templates.TemplateResponse(
            "root/erro.html",
            {"request": request, "usuario": usuario, "detail": ex.detail},
            status_code=ex.status_code,
        )

    @app.exception_handler(Exception)
    async def erro_exception(
        request: Request, ex: Exception, usuario: Usuario = Depends(obter_usuario_logado)
    ):
        return templates.TemplateResponse(
            "root/erro.html",
            {"request": request, "usuario": usuario, "detail": "Erro interno do servidor."},
            status_code=500,
        )
